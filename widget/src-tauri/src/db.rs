...
mod tests {
    ...
    fn create_comment_cli_actor_thread_and_shape() {
        let conn = test_db();
        let task = create_task(&conn, "CLI 评论任务", "todo", "none", None).unwrap();
        let id = task["id"].as_str().unwrap().to_string();
    ...
    fn list_comments_cli_orders_and_resolves_identifier() {
        let conn = test_db();
        let task = create_task(&conn, "列表任务", "todo", "none", None).unwrap();
        let id = task["id"].as_str().unwrap().to_string();
    ...
    fn update_and_delete_comment_cli_optimistic_lock() {
        let conn = test_db();
        let task = create_task(&conn, "乐观锁任务", "todo", "none", None).unwrap();
        let id = task["id"].as_str().unwrap().to_string();
    ...
    fn create_attachment_cli_derives_task_from_comment() {
        let conn = test_db();
        let task = create_task(&conn, "附件任务", "todo", "none", None).unwrap();
        let id = task["id"].as_str().unwrap().to_string();
