 ...
 * 跨列 FLIP：外层 motion wrapper（layoutId=任务 id）承担换列/重排平移；
 * 内层卡片保留 dragShift 内联 transform 与全部拖拽逻辑——motion 与拖拽
 * 预览互不争夺 transform（分工：外层动 layout，内层动 dragShift）。
 * ============================================================ */
...
export default function TaskCard({
  ...
  return (
    <motion.div layout="position" layoutId={task.id} transition={layoutTransition}>
      <div
        className={[
          'fb-card',
          `status-${task.status}`,
