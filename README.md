...
| 📋 **面板** | 360×520 | 点胶囊展开：任务列表、新建任务、一键流转状态、看详情写评论；详情信息结构与全版看板同口径（meta 行图标+标签摘要、描述→标签→评论分区），标题/描述/优先级/截止日点击就地编辑、标签勾选与新建（v0.4.0） |
...
| 要装什么 | 用来干嘛 | 下载地址 |
| --- | --- | --- |
| **Node.js 22.5 以上** | 仅在未构建挂件 exe、AI 命令行走 Node 回退脚本时需要 | [nodejs.org](https://nodejs.org)（选 LTS 版本即可） |
...
> 💡 v0.4.0 起 AI 命令行优先走挂件 exe 的 CLI 双模式（`taskdeck-widget.exe taskctl ...`），构建过 exe 就不需要 Node。如果暂时只想用 AI 命令行（不想要桌面挂件），装 Node.js 走回退路径也够。
