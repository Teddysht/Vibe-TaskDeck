...
- ~~taskctl 本地模式不支持 cloud / project map / relation / attachment~~ 部分清偿（2026-08-27，v0.3.5）：`issue relation add/remove` 与 `attachment upload/download` 已封装（写语义对齐 `db.rs`——端点规范化/环检测/单父替换/10MB 上限/UUID 磁盘存储；CLI 输出契约对齐上游 taskctl）。仍不支持：cloud / project map（纯客户端无 server，语义不适用）。
